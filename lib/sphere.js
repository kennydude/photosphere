...
Photosphere.prototype.loadBinary = function(callback){
	...
	// nickw 26/10/13
	// This is necessary to get binary data "peekable" (which exif.js needs)
	// otherwise the browser appears to do its own processing and reject byte 
	// values >=128; they are replaced with the value 253.
	// It is, I believe, stating that we're using our own character set
	// which allows all values from 0-255.

	// nickw 03/11/13
	// this doesn't work on IE (how predictable...), even IE10.
	// so only use it if supported. IE still gives some unhelpful system 
	// error on the last callback() call, so for the moment it's broken on IE.
...
Photosphere.prototype.loadEXIF = function(callback){
	...
	this.loadBinary(function(data){
		...
		// nickw 26/10/13
		// Use exif.js, if available, to get full exif data
		self.exif = typeof(EXIF)=="undefined" ? 
			{} : EXIF.readFromBinaryFile(new BinaryFile(data));
		...
		for(i in gpanoProperties)
			self.exif[i] = gpanoProperties[i];
